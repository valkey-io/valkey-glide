...
def create_servers(
    ...
    while len(servers_to_check) > 0:
        ...
        if is_address_already_in_use(server, f"{node_folder}/server.log", timeout=30):
            ...
            continue
        if not wait_for_server(server, cluster_folder, tls, 60, tls_cert_file, tls_key_file, tls_ca_cert_file):
            raise Exception(
