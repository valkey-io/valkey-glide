...
public sealed class NativeClient : IDisposable, INativeClient
{
    ...
    private static unsafe byte* MarshalBytes(byte[] bytes, byte* buffer = null, int bufferLength = 0)
    {
        ...
        if (bufferLength is 0 || bytes.Length > bufferLength - 2)
        {
            nint ptr = Marshal.AllocHGlobal(bytes.Length + 2);
            fixed (byte* bytesPtr = bytes)
            {
                Marshal.WriteByte(ptr, 0); // Is allocated on heap
                for (int i = 0; i < bytes.Length; i++)
                    Marshal.WriteByte(ptr + i + 1, bytesPtr[i]);
                Marshal.WriteByte(ptr + bytes.Length + 1, 0);
            }
            ...
            try
            {
                if (args.Length <= SmallStringOptimizationArgs)
                {
                    for (int i = 0; i < args.Length; i++)
                    {
                        ...
                        byte* ptr = MarshalBytes(args[i], buffer, SmallStringOptimizationBuffer);
                        argsArr[i] = ptr;
                    }
                }
                else
