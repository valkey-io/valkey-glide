...
public interface INativeClient
{
    ...
    /// Use <see cref="Encoding.UTF8"/> <see cref="Encoding.GetBytes(string)"/> to transform input strings.
    /// </param>
    /// <returns>A task representing the asynchronous operation, containing the result as a Value structure.</returns>
    Task<Value> SendCommandAsync<TRoutingInfo>(ERequestType requestType, TRoutingInfo routingInfo, params byte[][] args)
