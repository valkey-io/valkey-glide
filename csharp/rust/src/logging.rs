        ...
        let (pairs_ptr, pairs_length) = if values.is_empty() {
            ...
            for (key, value) in values.iter() {
                ...
                pairs.push(KeyValuePair {
                    key: helpers::to_cstr_ptr_or_null(key),
                    key_length: key.len() as c_int,
                    value: helpers::to_cstr_ptr_or_null(value.as_str()),
...
impl From<&Event<'_>> for SpanContext {
    fn from(value: &Event) -> Self {
        Self {
            kind: if value.is_root() { ESpanContextKind::Root } else if value.is_contextual() { ESpanContextKind::Current } else { ESpanContextKind::Explicit },
            parent_id: value.parent().map_or_else(|| 0, |v| v.into_u64() as c_ulonglong),
        }
    }
}
...
pub type EnterCallback = unsafe extern "C-unwind" fn(ref_data: *mut c_void, in_span_id: c_ulonglong);
