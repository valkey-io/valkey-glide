...
async fn hexpire_family() {
    let m = Mock::array(vec![ValkeyValue::Int(1), ValkeyValue::Int(1)]);
...
async fn httl_and_hpersist() {
    let m = Mock::array(vec![ValkeyValue::Int(100)]);
...
async fn hgetex_and_hsetex() {
    let m = Mock::array(vec![ValkeyValue::BulkString(b"v1".to_vec().into())]);
...
async fn httl_pttl_persist_expiretime() {
    let m = Mock::array(vec![ValkeyValue::Int(90)]);
    assert_eq!(m.httl("h", &["f1"]).await.unwrap(), vec![90]);
    m.assert_args(&["HTTL", "h", "FIELDS", "1", "f1"]);

    let m = Mock::array(vec![ValkeyValue::Int(90000)]);
    ...
    let m = Mock::array(vec![ValkeyValue::Int(1700000000)]);
    m.hexpiretime("h", &["f1"]).await.unwrap();
    m.assert_args(&["HEXPIRETIME", "h", "FIELDS", "1", "f1"]);

    let m = Mock::array(vec![ValkeyValue::Int(1700000000000)]);
