...
* Go: Add `AzAffinityAllNodes` read strategy (`ReadFrom_AZAffinityAllNodes`, proto value 6) — spreads reads round-robin across all nodes (primary and replicas) in the client's AZ, falling back to round-robin across all nodes when no in-AZ node is available ([#6792](https://github.com/valkey-io/valkey-glide/issues/6792))
