...
* Core/FFI: Re-authenticate a pool-borrowed client whose IAM token rotated while it sat idle, before it is lent, so a borrower never runs commands on a connection still authenticated with a stale token; the client is discarded if re-authentication fails. Previously the background refresh only updated the password used on the next reconnect, so a borrowed pooled client could run on the old (possibly expired) token until it happened to reconnect ([#6885](https://github.com/valkey-io/valkey-glide/issues/6885))
