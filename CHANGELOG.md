...
* CI/Node: Fix Windows npm platform package publishing — add the missing `node/npm/win32-x64-msvc/` platform stub so the Windows native module is published to npm on each release; fix `deprecate-on-failure` which was targeting the non-existent `@valkey/valkey-glide-windows-x64-msvc` instead of the correct `@valkey/valkey-glide-win32-x64-msvc`; fix `prepublishOnly` script which used removed napi-rs CLI v3 flags (`--config` → `--package-json-path`, `--skip-gh-release` → `--no-gh-release`); move `napi prepublish` out of the `prepublishOnly` npm lifecycle hook into an explicit CI step (napi-rs v3.8.3 workspace boundary check prevents running from the npm lifecycle cwd `node/npm/glide/`) and add `workspaces: ["npm/*"]` to `node/package.json` so the boundary check passes ([#7179](https://github.com/valkey-io/valkey-glide/pull/7179))
