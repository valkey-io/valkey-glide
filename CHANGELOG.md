...
* FFI: Prevent process abort (DoS) from malformed RESP3 pub/sub push frames in the direct-callback path. A push frame with non-BulkString elements or fewer than the expected element count triggered `unreachable!()` or index-out-of-bounds panics; with `panic = "abort"` in the release profile this crashed the entire host process. Malformed frames are now silently dropped, matching the async path's existing safe handling. ([#PENDING](https://github.com/valkey-io/valkey-glide/pull/PENDING))
