...
* Java: Pool-borrowed clients honor the configured inflight-requests limit and request timeout instead of the pool's defaults. `ClientPool.getClient` now resolves both from the client configuration (falling back to the core defaults) the same way directly-created clients do, so a pooled client's host-side fast-fail limiter and per-command timeout no longer silently differ from the same config used directly ([#7153](https://github.com/valkey-io/valkey-glide/issues/7153))
