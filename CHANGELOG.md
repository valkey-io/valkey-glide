...
* Python: route `test_cluster_batch` through the `exec_batch` retry wrapper and retry the transient RESP3+cluster atomic-pipeline `None` regardless of `raise_on_error`, matching the known transient aggregator artifact under concurrent multi-client test load ([#6719](https://github.com/valkey-io/valkey-glide/pull/6719))
