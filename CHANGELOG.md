...
* Core/FFI: Scoped command execution resolves its parent client from the scope registry instead of scanning the scope pools with `try_lock`. Under pool contention the scan reported no parent, and the command then ran with no request timeout, circuit breaker, inflight limit, IAM re-authentication or compression; the parent is now resolved without taking a pool lock and an unresolvable parent fails the command ([#6796](https://github.com/valkey-io/valkey-glide/issues/6796))
