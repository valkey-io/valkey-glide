...
* Python: An async client that is garbage-collected without `close()` now closes its connection. The client registry held a strong reference to every client, so clients created on short-lived event loops (`asyncio.run()` in a worker thread) kept their server connections open until the process exited ([#7122](https://github.com/valkey-io/valkey-glide/issues/7122))
