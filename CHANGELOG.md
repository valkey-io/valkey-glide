...
* CI/Node: Fix Windows npm platform package publishing — add the missing `node/npm/win32-x64-msvc/` platform stub so the Windows native module is published to npm on each release, and fix `deprecate-on-failure` which was targeting the non-existent `@valkey/valkey-glide-windows-x64-msvc` instead of the correct `@valkey/valkey-glide-win32-x64-msvc` ([#7179](https://github.com/valkey-io/valkey-glide/pull/7179))
