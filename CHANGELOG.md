...
* Core: Reset `CLIENT SETNAME` on scope release so a connection name set during a scope does not leak to the next borrower of the same connection ([#6898](https://github.com/valkey-io/valkey-glide/issues/6898))
* Core: Reused scoped connections keep the parent client's current runtime database. Acquire and release no longer discard the connection's tracked database, and release restores the parent's runtime database (captured per borrow) rather than the pool's original static configuration — so a reused scope reads and writes the same logical database as a newly opened one, and an untouched scope on a non-zero database takes the zero-round-trip clean release path ([#7064](https://github.com/valkey-io/valkey-glide/issues/7064))
* Core: Scoped connections work against IPv6 targets. The scope connection builder constructed a `redis://` URL and re-parsed it, which rejected a bare IPv6 host (`redis://::1:7801`) and surfaced only an eventual pool-exhaustion timeout; it now builds a `redis::ConnectionInfo` directly like every other connection path, reusing the cluster path's host/port parse so the two cannot drift ([#7124](https://github.com/valkey-io/valkey-glide/issues/7124))
...
* Core: Re-authenticate a pool-borrowed client whose IAM token rotated while it sat idle, on its first command after the borrow, so a borrower never runs a command on a connection still authenticated with a stale token. Previously the background refresh only updated the password used on the next reconnect, so a borrowed pooled client could run on the old (possibly expired) token until it happened to reconnect ([#6885](https://github.com/valkey-io/valkey-glide/issues/6885))
...
* Java: Pool-borrowed clients honor the configured request timeout and credentials instead of the pool's defaults. `ClientPool.getClient` now resolves the request timeout, inflight-requests limit, and credentials from the client configuration the same way directly-created clients do, so a pooled client's per-command timeout and IAM guards no longer silently differ from the same config used directly, and its host-side inflight limiter is configured from the client's limit rather than left off ([#7153](https://github.com/valkey-io/valkey-glide/issues/7153))
* Java: `ClientPool` now builds its `ConnectionRequest` through the same serializer as directly-created clients, so pooled clients (and scopes taken on them) honor every field carried in the connection request instead of silently dropping the ones the pool's separate serializer never mirrored (circuit breaker, `lazyConnect`, reconnect `jitterPercent`, mTLS, compression, client-side cache and more). Pooled clients now also resolve `connectionTimeout` the same way directly-created clients do rather than reusing `requestTimeout`. The address resolver and a custom IAM credentials provider remain unsupported for pooled clients — they are per-client Java callbacks the pool cannot forward, and a pool configured with either is now rejected rather than silently ignoring the callback or falling back to the default credential chain ([#6897](https://github.com/valkey-io/valkey-glide/issues/6897))
...
* CI: Add Windows CI support for the Node.js client. ([#7026](https://github.com/valkey-io/valkey-glide/pull/7026))
