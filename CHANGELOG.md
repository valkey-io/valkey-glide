...
* Python: Stabilize the systemic Python full-matrix (FMT) test failures caused by fire-and-forget `CLIENT KILL` calls timing out under heavy CI contention. Every disconnect-and-recover test issued `kill_connections(...)` unguarded; that command can transiently time out even though it reached the server (in cluster mode it is fanned over `AllNodes` and can sever the caller's own connections mid-fanout), which failed the test at the kill itself rather than at the recovery assertion. Added shared `kill_connections_tolerant` / `sync_kill_connections_tolerant` test helpers that tolerate a `TimeoutError` from the kill (the disconnect still applied) while letting any other error propagate, and applied them across the auth (`test_update_connection_password`) and pub/sub (`test_resubscribe_after_connection_kill_*`) tests, async and sync. Also widened the cluster-mode reconnect/resubscribe `wait_for` budgets so worst-case contended recovery fits; standalone budgets are unchanged. ([#6495](https://github.com/valkey-io/valkey-glide/issues/6495))
