...
* Core: Fix client hang when reconnect strategy `number_of_retries` is 0; the client now reconnects at a constant `factor * exponent_base` delay. Also fixes a reconnect stall when `number_of_retries` is very large ([#6379](https://github.com/valkey-io/valkey-glide/issues/6379))
