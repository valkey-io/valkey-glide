...
* CI: Fix `lint-ts` on the release-2.4 branch by committing a root `package-lock.json` pinning `prettier@3.8.4` and switching the lint workflow install step from `npm install` to `npm ci`, matching main's dependency-pinning posture. This stops Prettier from floating to 3.9.x, whose changed TypeScript union-type formatting broke `prettier --check` on pre-existing `node/*.ts` files.
