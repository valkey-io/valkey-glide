...
const ENV_GLIDE_LOG: &str = "GLIDE_LOG";
...
pub fn directory_from_env(envname: &str) -> Option<String> {
    let Ok(dirpath) = std::env::var(envname) else {
        return None;
    };

    if dirpath.trim().is_empty() {
        return None;
    }

    if std::fs::create_dir_all(&dirpath).is_ok() {
        Some(dirpath)
    } else {
        None
    }
...
pub fn init(minimal_level: Option<Level>, file_name: Option<&str>) -> Level {
    ...
    let reloads = INITIATE_ONCE.init_once.get_or_init(|| {
        ...
        let logs_dir = directory_from_env(ENV_GLIDE_LOG).unwrap_or(FILE_DIRECTORY.to_string());
...
mod tests {
    ...
    fn test_directory_from_env() {
        ...
        std::env::set_var(ENV_GLIDE_LOG, &dir_path);
        assert!(directory_from_env(ENV_GLIDE_LOG).is_some());
        ...
        std::env::set_var(ENV_GLIDE_LOG, "");
        assert!(directory_from_env(ENV_GLIDE_LOG).is_none());
    }
}
