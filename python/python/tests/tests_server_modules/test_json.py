...
class TestJson:
    ...
    @pytest.mark.parametrize("cluster_mode", [True, False])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    async def test_json_debug(self, glide_client: TGlideClient):
        key = get_random_string(10)
        ...
        # Test integer
        result = await json.debug(glide_client, "memory", key, "$.key1")
        assert result == [16]

        # Test float
        result = await json.debug(glide_client, "memory", key, "$.key2")
        assert result == [16]

        # Test Nested Value
        result = await json.debug(
            glide_client, "memory", key, "$.key3.nested_key.key1[0]"
        )
        assert result == [16]

        # Test Array
        result = await json.debug(glide_client, "memory", key, "$.key4[2]")
        assert result == [16]

        # Test String
        result = await json.debug(glide_client, "memory", key, "$.key6")
        assert result == [16]

        # Test Null
        result = await json.debug(glide_client, "memory", key, "$.key7")
        assert result == [16]

        # Test Bool
        result = await json.debug(glide_client, "memory", key, "$.key10")
        assert result == [16]

        # Test all keys
        result = await json.debug(glide_client, "memory", key, "$[*]")
        ...
        result = await json.debug(glide_client, "memory", key, "$.key11")
        assert result == []

        # Test for non-existent key
        result = await json.debug(glide_client, "memory", "non_existent_key", "$.key10")
        assert result == None
        ...
        # Test for non-existent path
        with pytest.raises(RequestError):
            await json.debug(glide_client, "memory", key, ".key11")
        ...
        # Test integer
        result = await json.debug(glide_client, "fields", key, "$.key1")
        assert result == [1]

        # Test float
        result = await json.debug(glide_client, "fields", key, "$.key2")
        assert result == [1]

        # Test Nested Value
        result = await json.debug(glide_client, "fields", key, "$.key3.nested_key.key1")
        assert result == [2]

        # Test Array
        result = await json.debug(glide_client, "fields", key, "$.key4[2]")
        assert result == [1]

        # Test String
        result = await json.debug(glide_client, "fields", key, "$.key6")
        assert result == [1]

        # Test Null
        result = await json.debug(glide_client, "fields", key, "$.key7")
        assert result == [1]

        # Test Bool
        result = await json.debug(glide_client, "fields", key, "$.key10")
        assert result == [1]

        # Test all keys
        result = await json.debug(glide_client, "fields", key, "$[*]")
        ...
        # Test for non-existent path
        result = await json.debug(glide_client, "fields", key, "$.key11")
        assert result == []

        # Test for non-existent key
        result = await json.debug(glide_client, "fields", "non_existent_key", "$.key10")
        assert result == None
        ...
        # Test for non-existent path
        with pytest.raises(RequestError):
            await json.debug(glide_client, "fields", key, ".key11")
