...
SYNC_OUTPUT_DIR = PYTHON_CLIENT_PATH
...
def build_sync_client(glide_root: Path, release: bool, no_cache: bool = False) -> None:
    print(f"[INFO] Building sync client in {'release' if release else 'debug'} mode...")
    prepare_python_env(glide_root, no_cache)
    ffi_dir = glide_root / "ffi"
    target_dir = ffi_dir / "target" / ("release" if release else "debug")
    lib_name = "libglide_ffi.so"

    build_cmd = ["cargo", "build"]
    if release:
        build_cmd += ["--release"]

    subprocess.run(build_cmd, cwd=ffi_dir, check=True)
    print("[OK] Rust build completed for sync client")

    built_lib = target_dir / lib_name
    dst_dir = glide_root / SYNC_OUTPUT_DIR
    dst_dir.mkdir(parents=True, exist_ok=True)
    copyfile(built_lib, dst_dir / lib_name)
    print(f"[OK] Copied {lib_name} to {dst_dir}")
...
def run_tests(glide_root: Path, extra_args: list[str] | None = None) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    python dev.py build all                      # Build both sync and async clients in debug mode (default)
    python dev.py build async --mode release     # Build the async client in release mode
    ...
    build_parser = subparsers.add_parser(
        "build", help="Build the async and/or sync Python clients"
    )
    build_parser.add_argument(
        "client",
        choices=["async", "sync", "all"],
        help="Which client to build: async, sync, or all",
    )
    build_parser.add_argument(
        "--mode",
    ...
    if args.command == "protobuf":
        ...
        if args.client in ("sync", "all"):
            ...
            build_sync_client(glide_root, release, no_cache)
