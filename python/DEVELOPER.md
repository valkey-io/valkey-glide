...
After installing prerequisites and cloning the repository, you can build the sync and/or async Python clients using the `dev.py` CLI utility.

### Examples:

```bash
# Build both sync and async clients in debug mode (default)
python python/dev.py build all

# Build the async client in release mode
python python/dev.py build async --mode release
```
...
This generates `.py` and `.pyi` interface files for type checking.
