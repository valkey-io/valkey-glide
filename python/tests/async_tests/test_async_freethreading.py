...
@pytest.mark.skipif(
    ...
    reason="Free-threading tests require Python built with --disable-gil (3.13t/3.14t)",
)
class TestAsyncFreeThreading:
    """Stress tests for async client free-threading safety."""

    @pytest.fixture(autouse=True)
    def _drain_shared_clients(self):
        """Close shared test-pooled clients to free connections for freethreading tests."""
        from tests.async_tests.conftest import _client_pool, _client_pool_lock
    ...
    async def _create_client_or_skip(self, config):
        """Create a client, skipping the test if the server is unreachable."""
        try:
            return await GlideClient.create(config)
        except Exception as e:
            pytest.skip(f"Client creation failed (server unreachable): {e}")
    ...
    async def test_concurrent_commands_single_client(self, request):
        ...
        client = await self._create_client_or_skip(config)
        try:
            num_tasks = 20
            errors = []
    ...
    async def test_multiple_clients_concurrent(self, request):
        ...
        clients = [
            await self._create_client_or_skip(config) for _ in range(num_clients)
        ]
        try:
