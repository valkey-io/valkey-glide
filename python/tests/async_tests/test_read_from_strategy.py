...
from glide_shared.config import ProtocolVersion, ReadFrom
from glide_shared.constants import OK
from glide_shared.routes import AllNodes, SlotIdRoute, SlotType
...
class TestAZAffinity:
    ...
    @pytest.mark.parametrize("cluster_mode", [True])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    async def test_az_affinity_all_nodes_requires_client_az(
        self, request, cluster_mode: bool, protocol: ProtocolVersion
    ):
    ...
    @pytest.mark.parametrize("cluster_mode", [True])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    async def test_az_affinity_all_nodes_splits_between_primary_and_replica(
        self,
        request,
        cluster_mode: bool,
        protocol: ProtocolVersion,
