...
class TestOpenTelemetryGlide:
    ...
    @pytest.mark.parametrize("cluster_mode", [True, False])
    async def test_trace_context_propagation_disabled_by_default(
        ...
        with use_parent_span(sampled=True):
            await client.get("GlideClient_test_propagation_disabled")
    ...
    async def test_out_of_range_parent_trace_flags_fall_back(
        ...
        with use_parent_span(sampled=True, trace_flags=trace_flags):
            await client.get(f"GlideClient_test_invalid_parent_flags_{trace_flags}")
    ...
    @pytest.mark.parametrize("cluster_mode", [True, False])
    async def test_max_parent_trace_flags_propagate(self, request, cluster_mode):
        """The upper byte boundary remains a valid remote parent."""
        client = await create_client(request, cluster_mode=cluster_mode)
        remove_span_file()

        with use_parent_span(sampled=True, trace_flags=255):
            await client.get("GlideClient_test_max_parent_flags")
