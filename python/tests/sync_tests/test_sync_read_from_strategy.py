...
class TestSyncAZAffinity:
    ...
    def test_sync_routing_with_az_affinity_strategy_to_1_replica(
        self,
        request,
        cluster_mode: bool,
        ...
        assert client_for_config_set.config_resetstat() == OK
    ...
    @pytest.mark.parametrize("cluster_mode", [True])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_routing_by_slot_to_replica_with_az_affinity_strategy_to_all_replicas(
        self,
        request,
        cluster_mode: bool,
        protocol: ProtocolVersion,
        ...
        azs = cast(
            ...
            client_for_testing_az.config_get(["availability-zone"], AllNodes()),
        )
        ...
        n_replicas = self._get_num_replicas(client_for_testing_az)
    ...
    @pytest.mark.parametrize("cluster_mode", [True])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_az_affinity_non_existing_az(
        self,
        request,
        cluster_mode: bool,
        protocol: ProtocolVersion,
        ...
        assert type(client_for_testing_az) is GlideClusterClient
        assert client_for_testing_az.config_resetstat() == OK
    ...
    @pytest.mark.parametrize("cluster_mode", [True])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_az_affinity_requires_client_az(
    ...
    @pytest.mark.parametrize("cluster_mode", [True])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_az_affinity_replicas_and_primary_routes_to_primary(
        self,
        request,
        cluster_mode: bool,
        protocol: ProtocolVersion,
