...
class TestScopeConnectionReuse:
    """Tests that scope connections are reused."""

    @pytest.mark.parametrize("cluster_mode", [True, False])
    def test_scope_connection_reuse(self, cluster_mode):
    ...
    @pytest.mark.parametrize("cluster_mode", [True, False])
    def test_scope_concurrent_multiple(self, cluster_mode):
        ...
        try:
            ...
            scope1 = client.scoped_connection().__enter__()
            scope2 = client.scoped_connection().__enter__()

            scope1.set(key1, "s1")
            scope2.set(key2, "s2")

            assert scope1.get(key1) == "s1"
            assert scope2.get(key2) == "s2"

            # WATCH conflict across scopes
            scope1.watch(key2)
            scope2.set(key2, "modified-by-s2")

            scope1.multi()
            scope1.set(key2, "from-s1")
            result = scope1.exec()
            assert result is None or result == "None"
    ...
    def test_scope_rejects_cross_slot_keys(self, cluster_mode):
        ...
        try:
            ...
            with client.scoped_connection() as scope:
                ...
                with pytest.raises(Exception, match="(?i)(cross.?slot|moved)"):
                    scope.set("{other-tag}-key2", "value2")
            client.delete([key1])
        finally:
            _close_client(client)

    @pytest.mark.parametrize("cluster_mode", [True])
    def test_scope_allows_same_slot_keys(self, cluster_mode):
        ...
        try:
            ...
            with client.scoped_connection() as scope:
                scope.set(key1, "v1")
                scope.set(key2, "v2")
                assert scope.get(key1) == "v1"
                assert scope.get(key2) == "v2"
            client.delete([key1, key2])
        finally:
            _close_client(client)

    @pytest.mark.parametrize("cluster_mode", [True])
    def test_scope_watch_multi_exec_cluster(self, cluster_mode):
    ...
    @pytest.mark.parametrize("cluster_mode", [True])
    def test_scope_different_routing_keys_cluster(self, cluster_mode):
        ...
        try:
            ...
            with client.scoped_connection(routing_key=key_a) as scope:
                scope.watch(key_a)
                val = scope.get(key_a)
                scope.multi()
                scope.set(key_a, str(int(val) + 1))
                result = scope.exec()
                assert result is not None

            # Second scope targets key_b's slot — must NOT reuse key_a's connection
            with client.scoped_connection(routing_key=key_b) as scope:
                scope.watch(key_b)
                val = scope.get(key_b)
                scope.multi()
                scope.set(key_b, str(int(val) + 1))
                result = scope.exec()
                assert result is not None

            assert client.get(key_a) == b"11"
            assert client.get(key_b) == b"21"
