...
class TestFreeThreading:
    ...
    def _drain_shared_clients(self):
        ...
        from tests.sync_tests.conftest import _sync_client_pool, _sync_client_pool_lock

        with _sync_client_pool_lock:
            for client in list(_sync_client_pool.values()):
                try:
                    client.close()
                except Exception:
                    pass
    ...
    def pool(self, request):
        ...
        try:
            p = ClientPool.create(
                config, PoolConfig(max_size=8, min_idle=4, acquire_timeout_s=30.0)
            )
        except Exception as e:
            pytest.skip(f"Pool creation failed (server unreachable): {e}")
        # Poll until pool has warmed up
    ...
    def test_parallel_pool_acquire_release_storm(self, pool):
        ...
        def worker(thread_id):
            for i in range(cycles_per_thread):
                try:
                    client_id = pool.acquire(timeout=30.0)
