...
class TestSyncReadOnlyMode:
    """Tests for read-only mode in standalone client (sync version)."""

    @pytest.mark.parametrize("cluster_mode", [False])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_read_only_mode_blocks_write_commands(
        self,
        request,
        cluster_mode: bool,
    ...
    @pytest.mark.parametrize("cluster_mode", [False])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_read_only_mode_allows_read_commands(
        self,
        request,
        cluster_mode: bool,
        protocol: ProtocolVersion,
    ...
    @pytest.mark.parametrize("cluster_mode", [False])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_read_only_rejects_az_affinity(
        self,
        request,
        cluster_mode: bool,
        protocol: ProtocolVersion,
    ...
    @pytest.mark.parametrize("cluster_mode", [False])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_read_only_rejects_az_affinity_replicas_and_primary(
        self,
        request,
        cluster_mode: bool,
        protocol: ProtocolVersion,
    ...
    @pytest.mark.parametrize("cluster_mode", [False])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_read_only_connects_to_primary_and_replica_separately(
        self,
        request,
        cluster_mode: bool,
        protocol: ProtocolVersion,
        ...
        client_node_0 = GlideClient.create(
            GlideClientConfiguration(
        ...
        client_node_1 = GlideClient.create(
            GlideClientConfiguration(
        ...
        try:
            ...
            result_0 = client_node_0.get("test_key_read_only")
            result_1 = client_node_1.get("test_key_read_only")
            ...
            with pytest.raises(RequestError) as exc_info_0:
                client_node_0.set("test_key", "value")
            assert (
                "write commands are not allowed in read-only mode"
            ...
            with pytest.raises(RequestError) as exc_info_1:
                client_node_1.set("test_key", "value")
            assert (
                "write commands are not allowed in read-only mode"
