...
def get_client_list_output_count(output: Union[bytes, str, None]) -> int:
...
def get_client_count(client: Union[GlideClient, GlideClusterClient]) -> int:
    ...
    if isinstance(client, GlideClusterClient):
        ...
        result_standalone: Any = client.custom_command(["CLIENT", "LIST"])
        return get_client_list_output_count(
            cast(Union[bytes, str, None], result_standalone)
        )


def get_expected_new_connections(
    ...
    if isinstance(client, GlideClusterClient):
        ...
        result = client.custom_command(["CLUSTER", "NODES"])
...
def dedicated_cluster(
    cluster_mode: bool,
) -> Generator[ValkeyCluster, None, None]:
    """
...
class TestSyncLazyConnection:
    ...
    def test_sync_lazy_connection_establishes_on_first_command(
        self,
        request: Any,
        cluster_mode: bool,
        ...
        try:
            ...
            monitoring_client = create_sync_client(
                request,
                cluster_mode=cluster_mode,
                protocol=protocol,
                lazy_connect=False,
            ...
            monitoring_client.ping()  # Ensure connection is established
            clients_before_lazy_init = get_client_count(monitoring_client)

            # 2. Create the "lazy" client using the same dedicated Valkey cluster
            lazy_glide_client = create_sync_client(
                request,
                cluster_mode=cluster_mode,
                protocol=protocol,
            ...
            clients_after_lazy_init = get_client_count(monitoring_client)
            ...
            ping_response: Any = lazy_glide_client.ping()
            ...
            clients_after_first_command = get_client_count(monitoring_client)
            expected_new_connections = get_expected_new_connections(monitoring_client)
            ...
            if lazy_glide_client:
                lazy_glide_client.close()
            if monitoring_client:
                monitoring_client.close()

    @pytest.mark.parametrize("cluster_mode", [False, True])
    def test_sync_lazy_connection_with_non_existent_host(
        self,
        request: Any,
        cluster_mode: bool,
        protocol: ProtocolVersion,
        ...
        try:
            ...
            try:
                ...
                lazy_client = create_sync_client(
                    request,
                    cluster_mode=cluster_mode,
                    protocol=protocol,
                    lazy_connect=True,  # Lazy
