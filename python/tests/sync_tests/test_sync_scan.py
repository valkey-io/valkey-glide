...
def is_cluster_ready(client: GlideClusterClient, count: int) -> bool:
    ...
    while True:
        ...
        cluster_info = client.custom_command(["CLUSTER", "INFO"])
        cluster_info_map = {}
...
class TestSyncScan:
    ...
    def test_sync_cluster_scan_cleaning_cursor(
        ...
        cursor = cast(
            ClusterScanCursor, (glide_sync_client.scan(ClusterScanCursor()))[0]
        )
        ...
        with pytest.raises(RequestError) as e_info:
            glide_sync_client.scan(new_cursor_with_same_id)
    ...
    def test_sync_cluster_scan_all_types(self, glide_sync_client: GlideClusterClient):
        ...
        glide_sync_client.mset({k: "value" for k in string_keys})
        ...
        for key in hash_keys:
            glide_sync_client.hset(key, {"field": "value"})
        ...
        for key in list_keys:
            glide_sync_client.lpush(key, ["value"])
        ...
        for key in zset_keys:
            glide_sync_client.zadd(key, {"value": 1})
        ...
        keys: List[bytes] = []
        while not cursor.is_finished():
            result = glide_sync_client.scan(cursor, type=ObjectType.STRING)
            cursor = cast(ClusterScanCursor, result[0])
        ...
        keys.clear()
        while not cursor.is_finished():
            result = glide_sync_client.scan(cursor, type=ObjectType.SET)
            cursor = cast(ClusterScanCursor, result[0])
            result_keys = result[1]
            keys.extend(cast(List[bytes], result_keys))
        ...
        keys.clear()
        while not cursor.is_finished():
            result = glide_sync_client.scan(cursor, type=ObjectType.HASH)
            cursor = cast(ClusterScanCursor, result[0])
            result_keys = result[1]
            keys.extend(cast(List[bytes], result_keys))
        ...
        keys.clear()
        while not cursor.is_finished():
            result = glide_sync_client.scan(cursor, type=ObjectType.LIST)
            cursor = cast(ClusterScanCursor, result[0])
            result_keys = result[1]
            keys.extend(cast(List[bytes], result_keys))
        ...
        keys.clear()
        while not cursor.is_finished():
            result = glide_sync_client.scan(cursor, type=ObjectType.ZSET)
            cursor = cast(ClusterScanCursor, result[0])
            result_keys = result[1]
            keys.extend(cast(List[bytes], result_keys))
        ...
        keys.clear()
        while not cursor.is_finished():
            result = glide_sync_client.scan(cursor, type=ObjectType.STREAM)
            cursor = cast(ClusterScanCursor, result[0])
            result_keys = result[1]
            keys.extend(cast(List[bytes], result_keys))
    ...
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_cluster_scan_non_covered_slots(
        self,
        ...
        key = get_random_string(10)
        glide_client_scoped.mset({f"{key}{i}": "value" for i in range(1000)})
        cursor = ClusterScanCursor()
        result = glide_client_scoped.scan(cursor)
        cursor = cast(ClusterScanCursor, result[0])
        assert not cursor.is_finished()
        glide_client_scoped.config_set({"cluster-require-full-coverage": "no"})
        # forget one server
        address_to_forget = glide_client_scoped._config.addresses[0]
        all_other_addresses = glide_client_scoped._config.addresses[1:]
        id_to_forget = glide_client_scoped.custom_command(
            ["CLUSTER", "MYID"],
            ByAddressRoute(address_to_forget.host, address_to_forget.port),
        )
        for address in all_other_addresses:
            glide_client_scoped.custom_command(
        ...
        is_cluster_ready(glide_client_scoped, len(all_other_addresses))
        ...
        while not cursor.is_finished():
            result = glide_client_scoped.scan(cursor, allow_non_covered_slots=True)
            cursor = cast(ClusterScanCursor, result[0])
    ...
    def test_sync_standalone_scan_all_types(self, glide_sync_client: GlideClient):
        ...
        (
            ...
            encoded_zset_keys,
            encoded_stream_keys,
        ) = self.create_keys(glide_sync_client)

        cursor = "0"
        keys: List[bytes] = []
        cursor = self.scan_string(
            glide_sync_client,
            encoded_string_keys,
            encoded_set_keys,
            encoded_hash_keys,
    ...
    def create_keys(self, glide_sync_client):
        key = get_random_string(10)
        string_keys = [f"{key}{i}" for i in range(100)]
        glide_sync_client.mset({k: "value" for k in string_keys})
        encoded_string_keys = list(map(lambda k: k.encode(), string_keys))
