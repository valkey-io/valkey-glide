...
class TestSyncScripts:
    ...
    @pytest.mark.parametrize("cluster_mode", [True, False])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_acl_cat(self, glide_sync_client: TGlideClient):
        # Test ACL CAT without category - should return list of categories
        categories = glide_sync_client.acl_cat()
        ...
        read_commands = glide_sync_client.acl_cat("read")
    ...
    @pytest.mark.parametrize("cluster_mode", [True, False])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_acl_users_and_whoami(self, glide_sync_client: TGlideClient):
        # Test ACL USERS - should return list of users
        users = glide_sync_client.acl_users()
        ...
        current_user = glide_sync_client.acl_whoami()
    ...
    @pytest.mark.parametrize("cluster_mode", [True, False])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_acl_list(self, glide_sync_client: TGlideClient):
        # Test ACL LIST - should return list of ACL rules
        acl_rules = glide_sync_client.acl_list()
    ...
    @pytest.mark.parametrize("cluster_mode", [True, False])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_acl_getuser(self, glide_sync_client: TGlideClient):
        # Test ACL GETUSER for default user
        user_info = glide_sync_client.acl_getuser("default")
        ...
        non_existent_user = glide_sync_client.acl_getuser("nonexistent_user_12345")
        assert non_existent_user is None

    @pytest.mark.skip_if_version_below("6.0.0")
    @pytest.mark.parametrize("cluster_mode", [True, False])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_acl_genpass(self, glide_sync_client: TGlideClient):
        # Test ACL GENPASS without bits parameter
        password1 = glide_sync_client.acl_genpass()
        assert isinstance(password1, bytes)
        assert len(password1) > 0

        # Test ACL GENPASS with bits parameter
        password2 = glide_sync_client.acl_genpass(32)
    ...
    @pytest.mark.parametrize("cluster_mode", [True, False])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_acl_setuser_and_deluser(self, glide_sync_client: TGlideClient):
        test_username = f"testuser_{get_random_string(8)}"

        try:
            # Test ACL SETUSER - create a new user
            result = glide_sync_client.acl_setuser(
            ...
            user_info = glide_sync_client.acl_getuser(test_username)
            assert user_info is not None
            assert isinstance(user_info, list)

            # Test ACL DELUSER
            deleted_count = glide_sync_client.acl_deluser(test_username)
            assert deleted_count == 1

            # Verify user was deleted
            users_after = glide_sync_client.acl_users()
            ...
            try:
                glide_sync_client.acl_deluser(test_username)
            except:
                pass
            raise
    ...
    @pytest.mark.parametrize("cluster_mode", [True, False])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_acl_dryrun(self, glide_sync_client: TGlideClient):
        # Test ACL DRYRUN with default user and GET command
        result = glide_sync_client.acl_dryrun("default", "GET", "somekey")
        assert isinstance(result, bytes)
        ...
        try:
            ...
            glide_sync_client.acl_setuser(
                test_username, "on", "nopass", "~*", "+get", "-@all"
            )

            # Test allowed command
            result_allowed = glide_sync_client.acl_dryrun(test_username, "GET", "key")
            assert result_allowed == b"OK"

            # Test disallowed command
            result_denied = glide_sync_client.acl_dryrun(
    ...
    @pytest.mark.parametrize("cluster_mode", [True, False])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_acl_log(self, glide_sync_client: TGlideClient):
        # Clear the log first
        reset_result = glide_sync_client.acl_log_reset()
        assert reset_result == b"OK"

        # Test ACL LOG without count
        log_entries = glide_sync_client.acl_log()
        assert isinstance(log_entries, list)
        # After reset, should be empty or have minimal entries

        # Test ACL LOG with count
        log_entries_limited = glide_sync_client.acl_log(5)
    ...
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_acl_save_load(self, glide_sync_client: GlideClient):
        # Note: These commands require server to be configured with aclfile
        # They may fail if not configured, which is expected

        try:
            # Test ACL SAVE
            save_result = glide_sync_client.acl_save()
            assert save_result == b"OK"

            # Test ACL LOAD
            load_result = glide_sync_client.acl_load()
    ...
    @pytest.mark.parametrize("cluster_mode", [True, False])
    @pytest.mark.parametrize("protocol", [ProtocolVersion.RESP2, ProtocolVersion.RESP3])
    def test_sync_acl_commands_return_types(self, glide_sync_client: TGlideClient):
