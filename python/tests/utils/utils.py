...
def get_client_certificate_path() -> str:
    """
    Return the filesystem path to the client certificate for path-based mTLS tests.
    ...
    return os.path.join(glide_home, "utils", "tls_crts", "server.crt")


def get_client_key_path() -> str:
    """
    Return the filesystem path to the client private key for path-based mTLS tests.

    The key is expected to be at utils/tls_crts/server.key relative to GLIDE_HOME_DIR.
    This file is automatically generated when TLS clusters are started.
    """
    import os
    ...
    return os.path.join(glide_home, "utils", "tls_crts", "server.key")
