...
def get_client_auth_certificate() -> bytes:
    """
    Load the dedicated client certificate for mTLS tests.
    Returns the certificate data in PEM format.

    Unlike :func:`get_client_certificate`, which repurposes the server cert,
    this returns the dedicated ``client.crt`` (signed by the same CA and
    carrying a ``clientAuth`` extendedKeyUsage). It is the certificate a client
    presents to a cluster started with ``--tls-auth-clients yes``, which
    genuinely verifies it.

    The certificate is expected to be at utils/tls_crts/client.crt relative to
    GLIDE_HOME_DIR. This file is automatically generated when TLS clusters are
    started.
    """
    import os
    ...
    client_cert_path = os.path.join(glide_home, "utils", "tls_crts", "client.crt")
    return load_client_certificate_from_file(client_cert_path)


def get_client_auth_key() -> bytes:
    """
    Load the dedicated client private key for mTLS tests.
    Returns the key data in PEM format.

    This is the private key paired with :func:`get_client_auth_certificate`.

    The key is expected to be at utils/tls_crts/client.key relative to
    GLIDE_HOME_DIR. This file is automatically generated when TLS clusters are
    started.
    """
    import os
    ...
    client_key_path = os.path.join(glide_home, "utils", "tls_crts", "client.key")
    return load_client_certificate_from_file(client_key_path)
