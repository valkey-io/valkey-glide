...
def get_all_functions_in_directory(
    directory: Path, exclude_filenames: Set[str]
) -> Dict[str, Set[str]]:
    """
    ...
    print("functions_by_file:", functions_by_file)  # Debugging output
    print("Total functions found:", len(functions_by_file))  # Debugging output
...
def compare_function_sets(
    ...
    async_functions = get_all_functions_in_directory(
        async_dir, set(excluded_filenames["async_only"])
    )
    sync_functions = get_all_functions_in_directory(
        sync_dir, set(excluded_filenames["sync_only"])
    )
    ...
    assert (
        not missing_in_async
    ), f"{error_message_prefix} in async:\n{fmt_missing(missing_in_async, sync_functions)}"
    assert (
        not missing_in_sync
    ), f"{error_message_prefix} in sync:\n{fmt_missing(missing_in_sync, async_functions)}"
