...
# Validate version format (vX.Y.Z-rcN)
if ! [[ $VERSION =~ ^v[0-9]+\.[0-9]+\.[0-9]+-rc[0-9]+$ ]]; then
    echo "Error: Version must be in format vX.Y.Z-rcN (e.g., v1.3.4-rc2)"
    exit 1
fi
...
if [ "$os" = "Linux" ]; then
        ...
        sed -i -e 's|"redis-cli",|CLI_COMMAND,|g' ../utils/cluster_manager.py # TEMPORARY FIX
else
        # MacOS
        sed -i '' -e 's|module github.com/valkey-io/valkey-glide/go|module github.com/valkey-io/valkey-glide/go-test-rc|' go.mod
