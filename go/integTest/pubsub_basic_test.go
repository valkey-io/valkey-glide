	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			if tt.clientType == ClusterClient {
				_, err = publisher.(*glide.ClusterClient).Publish(tt.channelName, tt.messageContent, tt.sharded)
			} else {
				_, err = publisher.(*glide.Client).Publish(tt.channelName, tt.messageContent)
			}
			assert.Nil(t, err)
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			for _, channel := range tt.channels {
				var err error
				if tt.clientType == ClusterClient {
					_, err = publisher.(*glide.ClusterClient).Publish(channel, tt.messageContent, false)
				} else {
					_, err = publisher.(*glide.Client).Publish(channel, tt.messageContent)
				}
				assert.Nil(t, err)
			}
			...
			var err error
			if tt.clientType == ClusterClient {
				_, err = publisher.(*glide.ClusterClient).Publish("other-channel", "should not receive", false)
			} else {
				_, err = publisher.(*glide.Client).Publish("other-channel", "should not receive")
			}
			assert.Nil(t, err)
...
func (suite *GlideTestSuite) TestPubSub_Basic_ManyChannels() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			for _, channelName := range tt.channelNames {
				if tt.clientType == ClusterClient {
					_, err := publisher.(*glide.ClusterClient).Publish(channelName, tt.messageContent, tt.sharded)
					assert.Nil(t, err)
				} else {
					_, err := publisher.(*glide.Client).Publish(channelName, tt.messageContent)
					assert.Nil(t, err)
				}
			}
...
func (suite *GlideTestSuite) TestPubSub_Basic_CombinedExactPattern() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			var err error
			if tt.clientType == ClusterClient {
				_, err = publisher.(*glide.ClusterClient).Publish(tt.exactChannel, tt.messageContent, false)
			} else {
				_, err = publisher.(*glide.Client).Publish(tt.exactChannel, tt.messageContent)
			}
			assert.Nil(t, err)
