...
func (suite *GlideTestSuite) TestMigrateWithOptions() {
	suite.runWithDefaultClients(func(client interfaces.BaseClientCommands) {
		...
		// Non-existent key returns "NOKEY" (not an error)
		result, err := client.MigrateWithOptions(ctx, "nonexistent.host", 6379, []string{nonExistentKey}, 0, 1000, *migrateOpts)
		suite.NoError(err)
		suite.Equal("NOKEY", result)

		// Existing key migrated to unreachable host returns an error
		client.Set(ctx, key, "value")
		_, err = client.MigrateWithOptions(ctx, "nonexistent.host", 6379, []string{key}, 0, 1000, *migrateOpts)
		suite.Error(err)
	})
}
