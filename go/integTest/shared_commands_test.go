...
func (suite *GlideTestSuite) TestZMPopWithCount() {
	if suite.serverVersion < "7.0.0" {
		suite.T().Skip("This feature is added in version 7.0.0")
	}
	...
	suite.runWithDefaultClients(func(client api.BaseClient) {
		...
		res1, err := client.ZMPopWithCount([]string{key1}, options.MIN, 2)
		assert.Nil(suite.T(), err)
		assert.True(suite.T(), res1.IsNil())
		...
		res4, err := client.ZMPopWithCount([]string{key1}, options.MIN, 2)
		assert.Nil(suite.T(), err)
		assert.Equal(suite.T(), key1, res4.Value().Key)
		assert.ElementsMatch(
			suite.T(),
		...
		res5, err := client.ZMPopWithCount([]string{key1}, options.MIN, 10)
		assert.Nil(suite.T(), err)
		assert.Equal(suite.T(), key1, res5.Value().Key)
		assert.ElementsMatch(
			suite.T(),
		...
		res6, err := client.ZMPopWithCount([]string{key2}, options.MAX, 1)
		assert.Nil(suite.T(), err)
		...
		// Pop from an empty key3
		res7, err := client.ZMPopWithCount([]string{key3}, options.MIN, 1)
		assert.Nil(suite.T(), err)
