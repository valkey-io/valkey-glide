...
func (suite *GlideTestSuite) TestModuleGetSetCommand() {
	...
	suite.verifyOK(glidejson.Set(context.Background(), client, key, "$", jsonValue))

	jsonGetResult, err := glidejson.Get(context.Background(), client, key)
	assert.NoError(t, err)
	assert.Equal(t, jsonValue, jsonGetResult.Value())

	jsonGetResultWithMultiPaths, err := glidejson.GetWithOptions(context.Background(),
	...
	jsonGetResult, err = glidejson.Get(context.Background(), client, "non_existing_key")
	assert.NoError(t, err)
	assert.True(t, jsonGetResult.IsNil())

	jsonGetResult, err = glidejson.GetWithOptions(context.Background(),
...
func (suite *GlideTestSuite) TestModuleGetSetCommandMultipleValues() {
	...
	suite.verifyOK(glidejson.Set(context.Background(), client, key, "$..c", "\"new_value\""))
...
func (suite *GlideTestSuite) TestModuleGetSetCommandConditionalSet() {
	...
	jsonSetResult, err := glidejson.SetWithOptions(context.Background(),
		client,
		key,
		"$",
		jsonValue,
...
func (suite *GlideTestSuite) TestModuleGetSetCommandFormatting() {
	...
	key := uuid.New().String()
	suite.verifyOK(
		glidejson.Set(context.Background(), client, key, "$", "{\"a\": 1.0, \"b\": 2, \"c\": {\"d\": 3, \"e\": 4}}"),
	)
	expectedGetResult := "[\n" +
		"  {\n" +
		"    \"a\": 1.0,\n" +
	...
	actualGetResult, err := glidejson.GetWithOptions(context.Background(),
		client,
		key,
	...
	expectedGetResult2 := "[\n~{\n~~\"a\":*1.0,\n~~\"b\":*2,\n~~\"c\":*{\n~~~\"d\":*3,\n~~~\"e\":*4\n~~}\n~}\n]"
	actualGetResult2, err := glidejson.GetWithOptions(context.Background(),
		client,
		key,
		*glideoptions.NewJsonGetOptionsBuilder().
			SetPaths([]string{"$"}).SetIndent("~").SetNewline("\n").SetSpace("*"),
