...
func checkResponseType(response *C.struct_CommandResponse, expectedType C.ResponseType, isNilable bool) error {
	...
	if !isNilable && response == nil {
		return glideErrors.NewRequestError(
			fmt.Sprintf(
...
func parseInterface(response *C.struct_CommandResponse) (any, error) {
	...
	return nil, glideErrors.NewRequestError("Unexpected return type from Valkey")
}
...
func (node mapConverter[T]) convert(data any) (any, error) {
	if data == nil {
		if node.canBeNil {
			...
			return nil, glideErrors.NewRequestError(fmt.Sprintf("Unexpected type received: nil, expected: map[string]%v", getType[T]()))
		}
	}
	...
	for key, value := range data.(map[string]any) {
		if node.next == nil {
			...
			valueT, ok := val.(T)
			if !ok {
				return nil, glideErrors.NewRequestError(fmt.Sprintf("Unexpected type of map element: %T, expected: %v", val, getType[T]()))
			}
			result[key] = valueT
		}
	}
...
func (node arrayConverter[T]) convert(data any) (any, error) {
	if data == nil {
		if node.canBeNil {
			...
			return nil, glideErrors.NewRequestError(fmt.Sprintf("Unexpected type received: nil, expected: []%v", getType[T]()))
		}
	}
	arrData := data.([]any)
	...
	for _, value := range arrData {
		if node.next == nil {
			...
			valueT, ok := val.(T)
			if !ok {
				return nil, glideErrors.NewRequestError(fmt.Sprintf("Unexpected type of array element: %T, expected: %v", val, getType[T]()))
			}
			result = append(result, valueT)
		}
	}
...
func handle2DStringArrayResponse(response *C.struct_CommandResponse) ([][]string, error) {
	...
	if !ok {
		return nil, glideErrors.NewRequestError(fmt.Sprintf("unexpected type: %T", converted))
	}
	return res, nil
}
...
func handleStringDoubleMapResponse(response *C.struct_CommandResponse) (map[string]float64, error) {
	...
	if !ok {
		return nil, glideErrors.NewRequestError(fmt.Sprintf("unexpected type of map: %T", converted))
	}
	return result, nil
}
...
func handleStringToStringArrayMapOrNilResponse(
	...
	return nil, glideErrors.NewRequestError(fmt.Sprintf("unexpected type received: %T", res))
}
...
func handleMapOfArrayOfStringArrayResponse(response *C.struct_CommandResponse) (map[string][][]string, error) {
	...
	if !ok {
		return nil, glideErrors.NewRequestError(fmt.Sprintf("unexpected type of second element: %T", converted))
	}
...
func handleXAutoClaimResponse(response *C.struct_CommandResponse) (models.XAutoClaimResponse, error) {
	...
	len := len(arr)
	if len < 2 || len > 3 {
		return null, glideErrors.NewRequestError(fmt.Sprintf("Unexpected response array length: %d", len))
	}
	converted, err := mapConverter[[][]string]{
		arrayConverter[[]string]{
			arrayConverter[string]{
	...
	claimedEntries, ok := converted.(map[string][][]string)
	if !ok {
		return null, glideErrors.NewRequestError(fmt.Sprintf("unexpected type of second element: %T", converted))
	}
	...
	if len == 3 {
		...
		if !ok {
			return null, glideErrors.NewRequestError(fmt.Sprintf("unexpected type of third element: %T", converted))
		}
	}
