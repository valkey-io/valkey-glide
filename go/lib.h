 ...
 */
typedef struct ResponseArena ResponseArena;
 ...
 */
typedef uint16_t (*AddressResolverCallback)(uintptr_t client_id,
                                            const uint8_t *host,
                                            uintptr_t host_len,
                                            uint16_t port,
 ...
 */
typedef uint8_t (*CredentialProviderCallback)(uintptr_t client_id,
                                              uint8_t *access_key_id_buf,
                                              uintptr_t access_key_id_buf_len,
                                              uintptr_t *access_key_id_len,
                                              uint8_t *secret_access_key_buf,
                                              uintptr_t secret_access_key_buf_len,
                                              uintptr_t *secret_access_key_len,
                                              uint8_t *session_token_buf,
                                              uintptr_t session_token_buf_len,
 ...
 */
typedef void (*MonitorCallback)(uintptr_t client_ptr,
                                double timestamp,
                                int64_t db,
 ...
 */
void noop_success_callback(uintptr_t _index, const struct CommandResponse *_msg);

/**
 * No-op failure callback — safe to call from any thread (no GIL needed).
 */
void noop_failure_callback(uintptr_t _index, const char *_msg, enum RequestErrorType _err_type);
 ...
 */
void free_pubsub_pointer_payload(uint8_t *ptr, uintptr_t len);
 ...
 */
void init_async_pipe(int32_t pipe_write_fd);
 ...
 */
void reinit_async_pipe(int32_t pipe_write_fd);
 ...
 * * If `pubsub_callback` is non-zero, it must be a valid function pointer that lives while the client is open/active.
 */
const struct ConnectionResponse *create_client_from_uri(const char *uri_str,
                                                        const char *extra_options_json,
                                                        const struct ClientType *client_type,
                                                        PubSubCallback pubsub_callback);
 ...
 */
void free_response_arena(struct ResponseArena *arena_ptr);
 ...
 */
struct CommandResult *command_with_buffer(const void *client_adapter_ptr,
                                          uintptr_t request_id,
                                          enum RequestType command_type,
                                          unsigned long arg_count,
                                          const uintptr_t *args,
 ...
 */
struct CommandResult *command_with_buffers(const void *client_adapter_ptr,
                                           uintptr_t request_id,
                                           enum RequestType command_type,
                                           unsigned long arg_count,
                                           const uintptr_t *args,
                                           ...
                                           const uint8_t *route_bytes,
                                           uintptr_t route_bytes_len,
                                           uint8_t *const *response_bufs,
                                           const uintptr_t *response_buf_lens,
                                           uintptr_t response_buf_count,
                                           uint64_t span_ptr);
 ...
 * * This function should only be called with a `client_adapter_ptr` created by [`create_client`], before [`close_client`] was called with the pointer.
 */
struct CommandResult *refresh_iam_token(const void *client_adapter_ptr,
                                        uintptr_t request_id);
 ...
 * * This function should only be called with a `client_adapter_ptr` created by [`create_client`], before [`close_client`] was called with the pointer.
 */
struct CommandResult *get_cache_metrics(const void *client_adapter_ptr,
                                        uintptr_t request_id,
 ...
 */
uint64_t create_otel_span_with_trace_context(enum RequestType request_type,
                                             const char *trace_id,
                                             const char *span_id,
                                             uint8_t trace_flags,
 ...
 * * Any non-null string pointer must point to a valid, null-terminated UTF-8 C string.
 */
uint64_t create_batch_otel_span_with_trace_context(const char *trace_id,
                                                   const char *span_id,
                                                   uint8_t trace_flags,
                                                   const char *trace_state);
 ...
 */
const char *register_pubsub_callback(const void *client_adapter_ptr,
                                     PubSubCallback pubsub_callback);
 ...
 * * `client_adapter_ptr` must be a valid client pointer from create_client
 */
const char *unregister_pubsub_callback(const void *client_adapter_ptr);
 ...
 */
const struct ConnectionResponse *create_monitor_client(const uint8_t *connection_request_bytes,
                                                       uintptr_t connection_request_len,
 ...
 * * This function should only be called with a `client_adapter_ptr` created by [`create_client`], before [`close_client`] was called with the pointer.
 */
struct CommandResult *command_with_route_info(const void *client_adapter_ptr,
                                              uintptr_t request_id,
                                              enum RequestType command_type,
                                              unsigned long arg_count,
                                              const uintptr_t *args,
 ...
 */
const void *glide_pool_get_client_ptr(uint64_t client_id);
#endif
 ...
 */
int32_t glide_pool_metrics(uint64_t pool_id,
                           uint32_t *idle_out,
                           uint32_t *active_out,
                           uint32_t *total_out);
#endif
 ...
 */
int32_t glide_scope_execute_async(uint64_t scope_id,
 ...
 * `connection_request_ptr` must point to `connection_request_len` valid bytes.
 */
void glide_scope_prewarm(uint64_t client_id,
                         const uint8_t *connection_request_ptr,
                         uintptr_t connection_request_len,
                         uint32_t min_idle);
#endif
 ...
 * `connection_request_ptr` must point to `connection_request_len` valid bytes.
 */
int64_t glide_scope_try_acquire(uint64_t client_id,
                                const uint8_t *connection_request_ptr,
                                uintptr_t connection_request_len,
