...
function loadNativeBinding() {
    ...
    try {
        ...
        console.debug(
            "Local rust-client module not available, trying platform-specific packages",
        );
    }
            ...
            case "linux":
                switch (arch) {
                    case "x64":
                        if (isMusl()) {
                            nativeBinding = require(
                                `${packagePrefix}-linux-x64-musl`,
                            );
                        } else {
                            nativeBinding = require(
                                `${packagePrefix}-linux-x64-gnu`,
                            );
                        }
                    ...
                    case "arm64":
                        if (isMusl()) {
                            nativeBinding = require(
                                `${packagePrefix}-linux-arm64-musl`,
                            );
                        } else {
                            nativeBinding = require(
                                `${packagePrefix}-linux-arm64-gnu`,
                            );
                        }
                    ...
                    case "arm":
                        if (isMusl()) {
                            nativeBinding = require(
                                `${packagePrefix}-linux-arm-musleabihf`,
                            );
                        } else {
                            nativeBinding = require(
                                `${packagePrefix}-linux-arm-gnueabihf`,
                            );
                        }
...
export const DEFAULT_REQUEST_TIMEOUT_IN_MILLISECONDS =
    nativeBinding.DEFAULT_REQUEST_TIMEOUT_IN_MILLISECONDS;
export const DEFAULT_CONNECTION_TIMEOUT_IN_MILLISECONDS =
    nativeBinding.DEFAULT_CONNECTION_TIMEOUT_IN_MILLISECONDS;
export const DEFAULT_INFLIGHT_REQUESTS_LIMIT =
    nativeBinding.DEFAULT_INFLIGHT_REQUESTS_LIMIT;
