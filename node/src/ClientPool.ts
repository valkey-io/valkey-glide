...
export class ClientPool {
    ...
    private async buildClientForId(clientId: number): Promise<BaseClient> {
        ...
        // Without this, calling close() on a pool-acquired client would leak
        // the in_use entry permanently.
        const originalClose = (
            client as unknown as { close: () => void }
        ).close.bind(client);

        (client as unknown as { close: () => Promise<void> }).close =
            async () => {
                if (this.closed) {
                    // Pool already destroyed — just do the underlying close.
                    originalClose();
                } else {
                    // Route through pool release to properly reset state and
