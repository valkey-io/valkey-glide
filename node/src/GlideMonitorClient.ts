...
export type { MonitorLine } from "../build-ts/native";
import type { MonitorLine } from "../build-ts/native";
...
export class GlideMonitorClient {
    ...
    /**
     * Creates a GlideMonitorClient that streams server-side commands via the MONITOR command.
     *
     * Note: RESP2 is always used for monitor connections regardless of the protocol set in options,
     * because MONITOR streams plain-text inline responses incompatible with RESP3 push messages.
     *
     * @param options - Connection configuration.
     * @param callback - Optional callback invoked for each monitor line. If omitted, use {@link getNextMessage} or {@link tryGetNextMessage}.
     */
    static async create(
        ...
        const bytes = connection_request.ConnectionRequest.encode(
            request,
        ).finish();
