...
/// Also refreshes activity before clearing so the abandon monitor never observes
/// `is_blocking=false` with a stale `borrowed_at` timestamp.
struct UnmarkOnDrop(
...
impl Drop for UnmarkOnDrop {
    fn drop(&mut self) {
        if let Some(arc) = self.0.take() {
            // Refresh activity BEFORE clearing so the monitor sees current
            // borrowed_at even if it observes is_blocking=false momentarily.
