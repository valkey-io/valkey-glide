...
export class SocketConnection {
  ...
  private getCallbackIndex(): number {
    return (
      this.availableCallbackSlots.pop() ?? this.promiseCallbackFunctions.length
    );
  }
  ...
  private chainNewWriteOperation(
    ...
    this.previousOperation = this.previousOperation.then(
      () =>
        new Promise((resolve) => {
          const headerLength = this.getHeaderLength(secondString !== undefined);
          ...
          if (
            ...
            this.backingWriteBuffer = new ArrayBuffer(nextPow2(requiredLength));
          }
          ...
          this.writeHeaderToWriteBuffer(
            ...
            operationType,
            headerLength,
            secondString !== undefined ? firstStringLength : undefined
          );

          const uint8Array = new Uint8Array(this.backingWriteBuffer, 0, length);
