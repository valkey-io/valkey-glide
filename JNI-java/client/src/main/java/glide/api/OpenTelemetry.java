...
public class OpenTelemetry {
    ...
    private static void internalInit(OpenTelemetryConfig config) {
        ...
        // ===== Validation (mirror old Java behavior expected by tests) =====
        if (flushIntervalMs <= 0) {
            throw new IllegalArgumentException("flushIntervalMs must be a positive integer");
        }
        if (tracesEndpoint != null && tracesSamplePercentage < 0) {
        ...
        // Eager-create file for file exporter to avoid test read races
        if (tracesEndpoint != null && tracesEndpoint.startsWith("file://")) {
            Path p = Paths.get(tracesEndpoint.substring("file://".length()));
            try {
                if (p.getParent() != null) {
                    if (!Files.exists(p.getParent()) || !Files.isDirectory(p.getParent())) {
                        throw new IllegalArgumentException(
                                "The directory does not exist or is not a directory: " + p.getParent());
                    }
                }
                if (!Files.exists(p)) {
                    Files.createFile(p);
                ...
                // Swallow file create errors; native side will attempt again
            }
        }
    ...
    private static void validateEndpoint(String endpoint) {
        if (endpoint.startsWith("file://")) {
            ...
            Path path = Paths.get(endpoint.substring("file://".length()));
