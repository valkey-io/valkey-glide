use glide_ffi::{create_otel_span, create_named_otel_span, create_otel_span_with_parent, drop_otel_span};
...
fn test_create_otel_span_with_valid_inputs() {
    ...
    for request_type in request_types {
        ...
        assert_ne!(span_ptr, 0, "create_otel_span should succeed for {:?}", request_type);
        ...
        assert!(span_ptr >= 0x1000, "Span pointer should be above minimum valid address");
        assert!(span_ptr <= 0x7FFF_FFFF_FFFF_FFF8, "Span pointer should be below maximum valid address");
...
fn test_create_otel_span_with_invalid_inputs() {
    ...
    for request_type in invalid_request_types {
        let span_ptr = create_otel_span(request_type);
        assert_eq!(span_ptr, 0, "create_otel_span should fail for invalid request type {:?}", request_type);
    }
}
...
fn test_create_named_otel_span_with_valid_inputs() {
    ...
    for (name, should_succeed) in test_cases {
        ...
        if should_succeed {
            assert_ne!(span_ptr, 0, "create_named_otel_span should succeed for name: '{}'", name);
            ...
            assert_eq!(span_ptr, 0, "create_named_otel_span should fail for name: '{}'", name);
        }
    }
...
fn test_create_otel_span_with_parent_valid_inputs() {
    ...
    for request_type in request_types {
        ...
        assert_ne!(child_span_ptr, 0, "Child span creation should succeed for {:?}", request_type);
        
        // Verify child span has different pointer than parent
        assert_ne!(child_span_ptr, parent_span_ptr, "Child span should have different pointer than parent");
        
        // Verify pointer properties
        assert_eq!(child_span_ptr % 8, 0, "Child span pointer should be 8-byte aligned");
        assert!(child_span_ptr >= 0x1000, "Child span pointer should be above minimum valid address");
        assert!(child_span_ptr <= 0x7FFF_FFFF_FFFF_FFF8, "Child span pointer should be below maximum valid address");
...
fn test_create_otel_span_with_parent_invalid_inputs() {
    ...
    assert_ne!(child_with_null_parent, 0, "Null parent should fallback to independent span");
    ...
    for invalid_parent_ptr in invalid_parent_ptrs {
        ...
        assert_ne!(child_span_ptr, 0, "Invalid parent 0x{:x} should fallback to independent span", invalid_parent_ptr);
    ...
    let child_with_invalid_request = create_otel_span_with_parent(RequestType::InvalidRequest, valid_parent_ptr);
    assert_eq!(child_with_invalid_request, 0, "Invalid request type should return 0 even with valid parent");
...
fn test_ffi_functions_concurrent_access() {
    ...
    for thread_id in 0..num_threads {
        ...
        let handle = thread::spawn(move || {
            ...
            for i in 0..spans_per_thread {
                // Create named span
                let span_name = format!("thread_{}_span_{}", thread_id, i);
                ...
                if named_span_ptr != 0 {
                    ...
                    let child_span_ptr = create_otel_span_with_parent(RequestType::Get, named_span_ptr);
                    if child_span_ptr != 0 {
    ...
    assert!(total_spans_created > 0, "At least some spans should have been created");
    println!("Successfully created and cleaned up {} spans across {} threads", total_spans_created, num_threads);
}
...
fn test_span_hierarchy_creation() {
    ...
    assert_ne!(child1_ptr, child2_ptr, "Child spans should have different pointers");
    ...
    assert_ne!(grandchild1_ptr, 0, "Grandchild1 span creation should succeed");
    assert_ne!(grandchild2_ptr, 0, "Grandchild2 span creation should succeed");
    assert_ne!(grandchild1_ptr, grandchild2_ptr, "Grandchild spans should have different pointers");

    // Verify all spans have unique pointers
    let all_spans = vec![root_span_ptr, child1_ptr, child2_ptr, grandchild1_ptr, grandchild2_ptr];
    for i in 0..all_spans.len() {
        for j in (i + 1)..all_spans.len() {
            assert_ne!(all_spans[i], all_spans[j], "All spans should have unique pointers");
        }
    }
...
fn test_error_handling_and_logging() {
    ...
    assert_ne!(child_with_invalid_parent, 0, "Invalid parent should fallback to independent span");
...
fn test_boundary_conditions() {
    ...
    for (length, should_succeed) in boundary_cases {
        ...
        if should_succeed {
            assert_ne!(span_ptr, 0, "Name with {} characters should succeed", length);
            valid_spans.push(span_ptr);
        } else {
            assert_eq!(span_ptr, 0, "Name with {} characters should fail", length);
        }
    }
