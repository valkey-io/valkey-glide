...
#[test]
fn test_enhanced_error_handling_comprehensive() {
    ...
    let child_with_garbage_parent = create_otel_span_with_parent(RequestType::Get, 0xDEADBEEF);
    assert_ne!(child_with_garbage_parent, 0, "Garbage parent should fallback to independent span");

    // Test 7: create_otel_span_with_parent with invalid request type should return 0
    let child_with_invalid_request = create_otel_span_with_parent(RequestType::InvalidRequest, valid_span_ptr);
    assert_eq!(child_with_invalid_request, 0, "Invalid request type should return 0");

    // Test 8: create_otel_span_with_parent with valid parent should succeed
    let child_with_valid_parent = create_otel_span_with_parent(RequestType::Set, valid_span_ptr);
...
#[test]
fn test_string_validation_edge_cases() {
    // Initialize logger to capture error messages
    logger_core::init(Some(logger_core::Level::Debug), None);
    ...
    // Test with string at 257 characters (should fail)
