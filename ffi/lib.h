...
typedef struct ClusterScanCursor {
  const char *cursor;
} ClusterScanCursor;
...
struct ClusterScanCursor new_cluster_cursor(const char *new_cursor);
 ...
 * * Both the `success_callback` and `failure_callback` function pointers need to live while the client is open/active. The caller is responsible for freeing both callbacks.
 */
void request_cluster_scan(const void *client_adapter_ptr,
                          uintptr_t channel,
                          struct ClusterScanCursor cursor,
                          unsigned long arg_count,
                          const uintptr_t *args,
                          const unsigned long *args_len);
