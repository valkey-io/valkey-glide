...
pub unsafe extern "C-unwind" fn create_monitor_client(
    ...
    let monitor_client = match runtime.block_on(async {
        MonitorClient::new(&address, redis_conn_info, tls_mode, on_line).await
    }) {
        Ok(c) => c,
        Err(e) => {
            let err_msg = CString::new(format!("Failed to create monitor client: {e}"))
                .unwrap_or_default();
            return Box::into_raw(Box::new(ConnectionResponse {
                conn_ptr: std::ptr::null(),
                connection_error_message: err_msg.into_raw(),
