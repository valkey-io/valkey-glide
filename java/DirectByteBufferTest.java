...
public class DirectByteBufferTest {
    ...
    private static void testDirectByteBuffer(int size) {
        try {
            ...
            System.out.printf("%-8s: %-10s allocation in %6d μs [%s]%n", 
                sizeStr, "SUCCESS", allocTime, threshold);
    ...
    private static void testMemoryPressure() {
        ...
        try {
            for (int i = 0; i < buffers.length; i++) {
                ...
                if (i % 3 == 0) {
                    System.out.println("  Allocated " + (i + 1) + " buffers (" + formatSize(totalAllocated) + " total)");
                }
            }
            
            System.out.println("✅ Memory pressure test: PASSED (" + formatSize(totalAllocated) + " allocated)");
            
        } catch (OutOfMemoryError e) {
            System.out.println("⚠️  Memory pressure test: Hit memory limit at " + formatSize(totalAllocated));
        } catch (Exception e) {
