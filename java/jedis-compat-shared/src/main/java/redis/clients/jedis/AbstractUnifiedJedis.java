...
/**
 * Shared implementation for {@code UnifiedJedis} in the Jedis compatibility modules. This class
 * provides a unified API that works seamlessly with both standalone and cluster Valkey/Redis
 * servers, while leveraging Valkey GLIDE underneath for enhanced performance, reliability, and
 * feature support.
 *
...
public abstract class AbstractUnifiedJedis extends JedisCommon {
     ...
     * @throws JedisConnectionException if the connection cannot be established
     */
    protected AbstractUnifiedJedis(String host, int port) {
     ...
     * @throws JedisConnectionException if the connection cannot be established
     */
    protected AbstractUnifiedJedis(HostAndPort hostAndPort) {
     ...
     */
    protected AbstractUnifiedJedis(String url) {
        this(URI.create(url));
    }
     ...
     */
    protected AbstractUnifiedJedis(URI uri) {
        this(
     ...
     * @throws IllegalArgumentException if the URI format is invalid
     */
    protected AbstractUnifiedJedis(URI uri, JedisClientConfig config) {
    ...
    public ScanResult<String> scan(String cursor) {
        checkNotClosed();
        try {
            if (isClusterMode && !isJedis5CompatibilityLayer()) {
    ...
    public ScanResult<byte[]> scan(byte[] cursor) {
        ...
        try {
            ...
            String cursorStr = new String(cursor, cs);
            if (isClusterMode && !isJedis5CompatibilityLayer()) {
            ...
            if (isClusterMode) {
                ...
                if ("0".equals(cursorStr)) {
                    ...
                    List<byte[]> binaryKeys =
                            Arrays.stream(keys)
                                    .map(s -> s == null ? null : s.getBytes(cs))
                                    .collect(Collectors.toList());
                    return new ScanResult<>(nextCursor.toString().getBytes(cs), binaryKeys);
                } else {
                    ...
                    for (Object key : keys) {
                        keyList.add(key.toString().getBytes(cs));
                    }
                    return new ScanResult<>(nextCursor.getBytes(cs), keyList);
                }
            } else {
                ...
                List<byte[]> binaryKeys =
                        Arrays.stream(keys)
                                .map(s -> s == null ? null : s.getBytes(cs))
                                .collect(Collectors.toList());
                return new ScanResult<>(newCursor.getBytes(cs), binaryKeys);
            }
        } catch (InterruptedException | ExecutionException e) {
            throw new JedisException("SCAN operation failed", e);
        }
    ...
    public ScanResult<byte[]> scan(byte[] cursor, AbstractScanParams<?> params, byte[] type) {
        ...
        try {
            Charset cs = jedisBinaryCharset();
            String cursorStr = new String(cursor, cs);
            String typeStr = type != null ? new String(type, cs) : null;
            ...
            return new ScanResult<>(newCursor.getBytes(cs), keyList);
        } catch (InterruptedException | ExecutionException e) {
            throw new JedisException("SCAN operation failed", e);
        }
    }
    ...
    private ScanResult<byte[]> binaryScanResultFromStringScan(ScanResult<String> sr) {
        Charset cs = jedisBinaryCharset();
        List<byte[]> binaryKeys =
                sr.getResult().stream()
                        .map(s -> s == null ? null : s.getBytes(cs))
                        .collect(Collectors.toList());
        return new ScanResult<>(sr.getCursor().getBytes(cs), binaryKeys);
    }
