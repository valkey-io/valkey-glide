/** Copyright Valkey GLIDE Project Contributors - SPDX Identifier: Apache-2.0 */
package redis.clients.jedis.params;

import java.util.Objects;

/**
 * Parameters for ZRANGE command. Provides options for controlling ZRANGE behavior such as BYSCORE,
 * BYLEX, REV (reverse order), and LIMIT (pagination).
 */
public abstract class AbstractZRangeParams<T extends AbstractZRangeParams<T>> {

    @SuppressWarnings("unchecked")
    protected final T self() {
        return (T) this;
    }

    public enum ZRangeBy {
        INDEX,
        SCORE,
        LEX
    }

    private final ZRangeBy by;
    private final Object min;
    private final Object max;
    private boolean rev = false;
    private boolean limit = false;
    private int offset;
    private int count;

    protected AbstractZRangeParams() {
        throw new InstantiationError("Empty constructor must not be called.");
    }

    protected AbstractZRangeParams(int min, int max) {
        this.by = ZRangeBy.INDEX;
        this.min = min;
        this.max = max;
    }

    protected AbstractZRangeParams(double min, double max) {
        this.by = ZRangeBy.SCORE;
        this.min = min;
        this.max = max;
    }

    protected AbstractZRangeParams(ZRangeBy by, Object min, Object max) {
        if (by == null || by == ZRangeBy.SCORE || by == ZRangeBy.LEX) {
            // ok
        } else {
            throw new IllegalArgumentException(by.name() + " is not a valid ZRANGE type argument.");
        }
        this.by = by;
        this.min = min;
        this.max = max;
    }

    protected AbstractZRangeParams(ZRangeBy by, String min, String max) {
        this(by, (Object) min, (Object) max);
    }

    protected AbstractZRangeParams(ZRangeBy by, byte[] min, byte[] max) {
        this(by, (Object) min, (Object) max);
    }

    public T rev() {
        this.rev = true;
        return self();
    }

    public T limit(int offset, int count) {
        this.limit = true;
        this.offset = offset;
        this.count = count;
        return self();
    }

    public ZRangeBy getBy() {
        return by;
    }

    public Object getMin() {
        return min;
    }

    public Object getMax() {
        return max;
    }

    public boolean isRev() {
        return rev;
    }

    public boolean hasLimit() {
        return limit;
    }

    public int getOffset() {
        return offset;
    }

    public int getCount() {
        return count;
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (o == null || getClass() != o.getClass()) return false;
        AbstractZRangeParams<?> that = (AbstractZRangeParams<?>) o;
        return rev == that.rev
                && limit == that.limit
                && offset == that.offset
                && count == that.count
                && by == that.by
                && Objects.equals(min, that.min)
                && Objects.equals(max, that.max);
    }

    @Override
    public int hashCode() {
        return Objects.hash(by, min, max, rev, limit, offset, count);
    }
}
