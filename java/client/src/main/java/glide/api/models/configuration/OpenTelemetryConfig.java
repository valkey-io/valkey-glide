 ...
 *   <li>File: Use {@code file://} prefix followed by a full path to export the signals to a local
 *       file.
 *       <ul>
 *         <li>The {@code file://} endpoint supports both directory paths and explicit file paths:
 *             <ul>
 *               <li>If the path is a directory or lacks a file extension (e.g., {@code
 *                   file:///tmp/otel}), it will default to writing to a file named spans.json in
 *                   that directory (e.g., {@code /tmp/otel/spans.json}).
 *               <li>If the path includes a filename with an extension (e.g., {@code
 *                   file:///tmp/otel/traces.json}), the specified file will be used as-is.
 *               <li>The parent directory must already exist. If it does not, the client will fail
 *                   to initialize with an InvalidInput error.
 *               <li>If the target file already exists, new data will be appended to it (the file
...
public class OpenTelemetryConfig {
    ...
    /**
     * The duration in milliseconds the data will exported to the collector. If not specified, 5000
     * will be used.
     */
