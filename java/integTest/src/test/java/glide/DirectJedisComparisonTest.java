...
/**
 * Direct side-by-side comparison test that calls both GLIDE compatibility layer and validates
 * against expected actual Jedis behavior.
 *
 * <p>This test demonstrates the exact pattern you requested: 1. Call GLIDE Jedis compatibility
 * layer (compatibility.clients.jedis) 2. Compare with expected actual Jedis behavior 3. Validate
 * results are identical
 *
 * <p>NOTE: Package name updated from redis.clients.jedis to compatibility.clients.jedis to avoid
 * conflicts with actual Jedis and make it clear this is a compatibility layer.
 */
...
public class DirectJedisComparisonTest {
    ...
    static void setupClass() {
        ...
        System.out.println(
                "Calling GLIDE compatibility layer and comparing with expected Jedis behavior");
    }
    ...
    void directSetGetComparison() {
        ...
        System.out.println(
                "   ActualJedis.set('" + key + "', '" + value + "') = '" + expectedSetResult + "'");
        System.out.println("   ActualJedis.get('" + key + "') = '" + expectedGetResult + "'");
        System.out.println(
                "   ActualJedis.get('" + key + "_nonexistent') = " + expectedNonExistentResult);
        ...
        System.out.println(
                "   Non-existent: GLIDE="
                        + glideNonExistentResult
                        + " vs Expected="
                        + expectedNonExistentResult
                        + " -> "
                        + (glideNonExistentResult == expectedNonExistentResult
                                ? "IDENTICAL ✓"
                                : "DIFFERENT ✗"));

        // Assert they are identical
        assertEquals(
                expectedSetResult, glideSetResult, "SET results must be identical to actual Jedis");
        assertEquals(
                expectedGetResult, glideGetResult, "GET results must be identical to actual Jedis");
        assertEquals(
                expectedNonExistentResult,
                glideNonExistentResult,
                "Non-existent GET results must be identical to actual Jedis");
    ...
    void directPingComparison() {
        ...
        System.out.println(
                "   ActualJedis.ping('" + message + "') = '" + expectedPingMessageResult + "'");
        ...
        System.out.println(
                "   PING(message): GLIDE='"
                        + glidePingMessageResult
                        + "' vs Expected='"
                        + expectedPingMessageResult
                        + "' -> "
                        + (glidePingMessageResult.equals(expectedPingMessageResult)
                                ? "IDENTICAL ✓"
                                : "DIFFERENT ✗"));

        // Assert they are identical
        assertEquals(
                expectedPingResult, glidePingResult, "PING results must be identical to actual Jedis");
        assertEquals(
                expectedPingMessageResult,
                glidePingMessageResult,
                "PING(message) results must be identical to actual Jedis");
    ...
    void directPoolComparison() {
        ...
        System.out.println(
                "   GLIDEPool.getResource().set('"
                        ...
                        + "'");
        System.out.println(
                "   GLIDEPool.getResource().get('" + key + "') = '" + glidePoolGetResult + "'");
        ...
        System.out.println(
                "   ActualJedisPool.getResource().set('"
                        + key
                        + "', '"
                        + value
                        + "') = '"
                        + expectedPoolSetResult
                        + "'");
        System.out.println(
                "   ActualJedisPool.getResource().get('" + key + "') = '" + expectedPoolGetResult + "'");
        ...
        assertEquals(
                expectedPoolSetResult,
                glidePoolSetResult,
                "Pool SET results must be identical to actual JedisPool");
        assertEquals(
                expectedPoolGetResult,
                glidePoolGetResult,
                "Pool GET results must be identical to actual JedisPool");
    ...
    void directMultipleOperationsComparison() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            System.out.println(
                    "   GLIDE.set('" + keys[i] + "', '" + values[i] + "') = '" + glideSetResults[i] + "'");
        ...
        for (int i = 0; i < 3; i++) {
            ...
            assertEquals(
                    expectedSetResults[i],
                    glideSetResults[i],
                    "SET result " + (i + 1) + " must be identical to actual Jedis");
            assertEquals(
                    expectedGetResults[i],
                    glideGetResults[i],
                    "GET result " + (i + 1) + " must be identical to actual Jedis");
        }
    ...
    void finalDirectComparisonValidation() {
        ...
        System.out.println(
                "   GLIDE Direct: SET='"
                        + glideDirectSetResult
                        + "', GET='"
                        + glideDirectGetResult
                        + "', PING='"
                        + glideDirectPingResult
                        + "'");
        System.out.println(
                "   GLIDE Pool: SET='"
                        + glidePoolSetResult
        ...
        System.out.println(
                "   Expected Direct: SET='"
                        + expectedDirectSetResult
                        + "', GET='"
                        + expectedDirectGetResult
                        + "', PING='"
                        + expectedDirectPingResult
                        + "'");
        System.out.println(
                "   Expected Pool: SET='"
                        + expectedPoolSetResult
        ...
        assertEquals(
                expectedDirectSetResult, glideDirectSetResult, "Direct SET must match actual Jedis");
        assertEquals(
                expectedDirectGetResult, glideDirectGetResult, "Direct GET must match actual Jedis");
        assertEquals(
                expectedDirectPingResult, glideDirectPingResult, "Direct PING must match actual Jedis");
        ...
        assertEquals(
                expectedPoolPingResult, glidePoolPingResult, "Pool PING must match actual JedisPool");
        System.out.println("   ✓ Pool connection: ALL OPERATIONS IDENTICAL to actual JedisPool");

        // Final validation
        System.out.println("\n3. Final validation:");
        System.out.println(
                "✓ GLIDE Jedis compatibility layer produces IDENTICAL results to actual Jedis");
        System.out.println(
                "✓ Users can replace actual Jedis with GLIDE and get EXACTLY the same behavior");
