...
/**
 * Performance benchmark tests comparing GLIDE Jedis compatibility layer with actual Jedis
...
public class JedisPerformanceBenchmarkTest {
    ...
    void benchmarkSingleThreadedOperations() {
        ...
        BenchmarkResult glideResult =
                benchmarkGetSetOperations(glideJedis, "glide", DEFAULT_ITERATIONS);
        ...
        if (hasActualJedis) {
            ...
            BenchmarkResult actualResult =
                    benchmarkGetSetOperations(actualJedis, "actual", DEFAULT_ITERATIONS);
    ...
    void benchmarkMultiThreadedOperations() {
        ...
        // Benchmark GLIDE Jedis
        BenchmarkResult glideResult =
                benchmarkConcurrentOperations("glide", threadCount, operationsPerThread);
        printBenchmarkResult("GLIDE Jedis (Concurrent)", glideResult);

        // Benchmark actual Jedis (if available)
        if (hasActualJedis) {
            BenchmarkResult actualResult =
                    benchmarkConcurrentOperations("actual", threadCount, operationsPerThread);
    ...
    void benchmarkLargeValues() {
        ...
        for (int size : valueSizes) {
            ...
            // Benchmark GLIDE Jedis
            BenchmarkResult glideResult =
                    benchmarkLargeValueOperations(glideJedis, keyPrefix + "glide", largeValue, 1000);
            ...
            if (hasActualJedis) {
                ...
                BenchmarkResult actualResult =
                        benchmarkLargeValueOperations(actualJedis, keyPrefix + "actual", largeValue, 1000);
    ...
    void benchmarkPoolPerformance() {
        ...
        try {
            ...
            BenchmarkResult glidePoolResult =
                    benchmarkPoolOperations(glidePool, "glide_pool", DEFAULT_ITERATIONS);
            printBenchmarkResult("GLIDE JedisPool", glidePoolResult);

            closeClient(glidePool);

            // Compare with direct connection
            BenchmarkResult glideDirectResult =
                    benchmarkGetSetOperations(glideJedis, "glide_direct", DEFAULT_ITERATIONS);
            printBenchmarkResult("GLIDE Direct Connection", glideDirectResult);

            System.out.println(
                    "Pool vs Direct overhead: "
                            + String.format(
                                    "%.2f%%",
                                    ((double) (glidePoolResult.totalTimeNs - glideDirectResult.totalTimeNs)
                                                    / glideDirectResult.totalTimeNs)
                                            * 100));
    ...
    void benchmarkMemoryUsage() {
        ...
        try {
            ...
            System.out.println(
                    "GLIDE Jedis memory usage (100 connections): "
    ...
    private BenchmarkResult benchmarkGetSetOperations(
            Object jedis, String keyPrefix, int iterations) {
        long totalSetTime = 0;
        long totalGetTime = 0;
        long totalTime = 0;
    ...
    private BenchmarkResult benchmarkConcurrentOperations(
            String keyPrefix, int threadCount, int operationsPerThread) {
                            ...
                            () -> {
                                try {
                                    ...
                                    BenchmarkResult result =
                                            benchmarkGetSetOperations(
                                                    threadJedis, keyPrefix + "_thread_" + threadId, operationsPerThread);
    ...
    private void performLargeValueWarmup(
            Object jedis, String keyPrefix, String value, int iterations) {
        try {
            for (int i = 0; i < iterations; i++) {
    ...
    private BenchmarkResult benchmarkLargeValueOperations(
            Object jedis, String keyPrefix, String value, int iterations) {
    ...
    private void compareBenchmarkResults(
            String testName, BenchmarkResult glideResult, BenchmarkResult actualResult) {
