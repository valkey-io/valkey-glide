 ...
 *
 * <p>With the new package name (compatibility.clients.jedis), we can now directly test the
 * compatibility layer without complex reflection or classpath management.
 *
 * <p>This test demonstrates direct usage and validates behavior against expected Jedis patterns.
 */
...
public class SideBySideJedisComparisonTest {
    ...
    void directPoolUsageComparison() {
        ...
        System.out.println(
                "    jedis.set(\"" + key + "\", \"" + value + "\") = \"" + poolSetResult + "\"");
    ...
    void realWorldMigrationExample() {
        ...
        String[] keys = {
            TEST_KEY_PREFIX + "item1", TEST_KEY_PREFIX + "item2", TEST_KEY_PREFIX + "item3"
        };
    ...
    void performanceAndReliabilityValidation() {
        ...
        System.out.println(
                "Average: "
                        + String.format("%.2f", (double) duration / (operationCount * 2))
                        + "ms per operation");
