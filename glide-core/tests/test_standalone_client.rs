...
mod standalone_client_tests {
    ...
    fn test_select_command_database_persistence_after_reconnection() {
        ...
        block_on_all(async move {
            ...
            let initial_client_info: String = String::from_owned_redis_value(
                client.send_command(&client_info_cmd).await.unwrap(),
            )
            .unwrap();
            ...
            // Extract initial client ID
            let initial_client_id = extract_client_id(&initial_client_info)
            ...
            let post_select_client_info: String = String::from_owned_redis_value(
                client.send_command(&client_info_cmd).await.unwrap(),
            )
            .unwrap();
