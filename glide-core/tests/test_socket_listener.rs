...
mod socket_listener {
    ...
    use super::*;
    use glide_core::glide_request::command::{Args, ArgsArray};
    use glide_core::glide_request::{Command, Transaction};
    use glide_core::response::{response, ConstantResponse, Response};
    use glide_core::scripts_container::add_script;
    use glide_request::{GlideRequest, RequestType};
    ...
    fn get_command_request(
        ...
        request.command = Some(glide_request::glide_request::Command::SingleCommand(
    ...
    fn write_request(buffer: &mut Vec<u8>, socket: &mut UnixStream, request: GlideRequest) {
    ...
    fn write_transaction_request(
        ...
        request.command = Some(glide_request::glide_request::Command::Transaction(
            transaction,
        ));
    ...
    fn test_socket_pass_manual_route_to_all_primaries() {
        ...
        let mut routes = glide_request::Routes::default();
        routes.set_simple_routes(glide_request::SimpleRoutes::AllPrimaries);
    ...
    fn test_socket_cluster_route_by_address_reaches_correct_node() {
        ...
        let mut routes = glide_request::Routes::default();
        routes.set_simple_routes(glide_request::SimpleRoutes::Random);
        request.route = Some(routes).into();
        ...
        buffer.clear();
        let mut routes = glide_request::Routes::default();
        let by_address_route = glide_core::glide_request::ByAddressRoute {
            host: host.into(),
            port,
            ..Default::default()
        };
    ...
    fn test_send_script(
        ...
        let mut request = GlideRequest::new();
        request.callback_idx = CALLBACK_INDEX;
        request.command = Some(glide_request::glide_request::Command::ScriptInvocation(
