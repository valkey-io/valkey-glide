...
mod test_monitor {
    ...
    async fn test_monitor_start_and_receive_line() {
        ...
        let mut monitor = MonitorClient::new(&node_addr, monitor_conn_info(), TlsMode::NoTls, on_line)
            .await
        ...
        loop {
            ...
            assert!(std::time::Instant::now() < deadline, "timed out waiting for SET line");
        ...
        let set_line = found
            .iter()
            .find(|l| l.command == "SET" && l.args.first().map(|s| s.as_str()) == Some("monitor_test_key"))
            .unwrap();
