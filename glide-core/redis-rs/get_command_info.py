...
def main():
    ...
    multi_shard = CommandCategory(
        "MultiShardNoValues or MultiShardWithValues",
        "Commands with a MULTI_SHARD request policy",
    )
    first_arg = CommandCategory(
        "FirstKey", "Commands with their first key argument at position 1"
    )
    second_arg = CommandCategory(
        "SecondArg", "Commands with their first key argument at position 2"
    )
    second_arg_numkeys = CommandCategory(
    ...
    third_arg_numkeys = CommandCategory(
        "ThirdArgAfterKeyCount",
        "Commands with their first key argument at position 3, after a numkeys argument",
    )
    streams_index = CommandCategory(
        "StreamsIndex", "Commands that include a STREAMS token"
    )
    second_arg_slot = CommandCategory(
        "SecondArgSlot", "Commands with a slot argument at position 2"
    )
    ...
    for filename in os.listdir(commands_dir):
        ...
        if len(split_name) == 0:
            raise Exception(
                f"Encountered json with an empty command name in file '{filename}'"
            )
        ...
        if json_key_index == -1:
            ...
            cluster_routing_slot_index = (
                -1 if json_slot_index == -1 else len(split_name) + json_slot_index
            )
        ...
        # cluster_routing.rs considers each word in the command name to be an argument, but the json does not
        cluster_routing_key_index = (
            -1 if json_key_index == -1 else len(split_name) + json_key_index
        )
        ...
        elif cluster_routing_key_index == 3 and is_after_numkeys(
            command_args, json_key_index
        ):
...
def is_after_numkeys(args_info_json, json_index):
    return (
        json_index > 0 and args_info_json[json_index - 1]["name"].lower() == "numkeys"
    )
