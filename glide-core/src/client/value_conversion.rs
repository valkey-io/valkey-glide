                    ...
                    .map(|item| match item {
                        ...
                        _ => match from_owned_redis_value::<bool>(item.clone()) {
                            ...
                            _ => Err((
                                ...
                                format!("(response was {value_type:?})", value_type = get_value_type(&item)),
                            )
                                .into()),
                        },
                    })
                ...
                for aggregation in array.into_iter().skip(1) {
                    let Value::Array(fields) = aggregation else {
                        return Err((
                            ...
                            format!("(`fields` was {value_type:?})", value_type = get_value_type(&aggregation)),
                        )
                            .into());
                    };
            ...
            Value::Array(_) => {
                ...
                let Value::Array(fields) = fields_value else {
                    return Err((
                        ...
                        format!("(`fields` was {value_type:?})", value_type = get_value_type(&fields_value)),
                    ).into());
                };
