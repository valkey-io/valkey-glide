...
mod tests {
    ...
    mod protobuf_conversion_tests {
        ...
        #[test]
        fn test_compression_config_conversion_zero_min_compression_size() {
            let mut proto_request = protobuf::ConnectionRequest::new();
            proto_request.addresses.push(protobuf::NodeAddress {
                host: "localhost".into(),
                port: 6379,
            ...
            compression_config.enabled = true;
            compression_config.backend = protobuf::CompressionBackend::ZSTD.into();
            compression_config.min_compression_size = 0; // Edge case: zero
            ...
            let config = request.compression_config.unwrap();
            // Conversion should succeed, but validation will fail later
            assert_eq!(config.min_compression_size, 0);
            assert_eq!(config.backend, CompressionBackendType::Zstd);
        }

        #[test]
        fn test_compression_config_conversion_very_large_min_compression_size() {
            let mut proto_request = protobuf::ConnectionRequest::new();
            proto_request.addresses.push(protobuf::NodeAddress {
                host: "localhost".into(),
                port: 6379,
            ...
            compression_config.enabled = true;
            compression_config.backend = protobuf::CompressionBackend::LZ4.into();
            compression_config.min_compression_size = u32::MAX; // Edge case: maximum value
            ...
            let config = request.compression_config.unwrap();
            // Should convert u32::MAX to usize
            assert_eq!(config.min_compression_size, u32::MAX as usize);
            assert_eq!(config.backend, CompressionBackendType::Lz4);
        }

        #[test]
        fn test_compression_config_conversion_negative_compression_level() {
            let mut proto_request = protobuf::ConnectionRequest::new();
            proto_request.addresses.push(protobuf::NodeAddress {
                host: "localhost".into(),
                port: 6379,
            ...
            compression_config.enabled = true;
            compression_config.backend = protobuf::CompressionBackend::LZ4.into();
            compression_config.compression_level = Some(-5); // Valid for LZ4 fast mode
            compression_config.min_compression_size = 64;
            ...
            let config = request.compression_config.unwrap();
            // Negative levels should be preserved (LZ4 supports negative for fast mode)
            assert_eq!(config.compression_level, Some(-5));
            assert_eq!(config.backend, CompressionBackendType::Lz4);
        }

        #[test]
        fn test_compression_config_conversion_zero_compression_level() {
            let mut proto_request = protobuf::ConnectionRequest::new();
            proto_request.addresses.push(protobuf::NodeAddress {
                host: "localhost".into(),
                port: 6379,
            ...
            compression_config.enabled = true;
            compression_config.backend = protobuf::CompressionBackend::LZ4.into();
            compression_config.compression_level = Some(0); // LZ4 default level
            compression_config.min_compression_size = 64;
        ...
        #[test]
        fn test_compression_config_conversion_very_high_compression_level() {
            let mut proto_request = protobuf::ConnectionRequest::new();
            proto_request.addresses.push(protobuf::NodeAddress {
                host: "localhost".into(),
                port: 6379,
            ...
            compression_config.enabled = true;
            compression_config.backend = protobuf::CompressionBackend::ZSTD.into();
            compression_config.compression_level = Some(999); // Invalid, too high
            compression_config.min_compression_size = 64;
            ...
            let config = request.compression_config.unwrap();
            // Conversion should succeed, but validation will fail later
            assert_eq!(config.compression_level, Some(999));
            assert_eq!(config.backend, CompressionBackendType::Zstd);
        }

        #[test]
        fn test_compression_config_conversion_very_low_compression_level() {
            let mut proto_request = protobuf::ConnectionRequest::new();
            proto_request.addresses.push(protobuf::NodeAddress {
                host: "localhost".into(),
                port: 6379,
            ...
            compression_config.enabled = true;
            compression_config.backend = protobuf::CompressionBackend::ZSTD.into();
            compression_config.compression_level = Some(-999); // Invalid for Zstd
            compression_config.min_compression_size = 64;
        ...
        #[test]
        fn test_compression_config_conversion_disabled_with_invalid_values() {
            let mut proto_request = protobuf::ConnectionRequest::new();
            proto_request.addresses.push(protobuf::NodeAddress {
                host: "localhost".into(),
                port: 6379,
            ...
            let mut compression_config = protobuf::CompressionConfig::new();
            compression_config.enabled = false; // Disabled
            compression_config.backend = EnumOrUnknown::from_i32(999); // Invalid backend
            compression_config.compression_level = Some(999); // Invalid level
            compression_config.min_compression_size = 0; // Invalid size
            ...
            let config = request.compression_config.unwrap();
            // Even with invalid values, conversion should succeed when disabled
            assert!(!config.enabled);
            assert_eq!(config.backend, CompressionBackendType::Zstd); // Fallback
            assert_eq!(config.compression_level, Some(999));
            assert_eq!(config.min_compression_size, 0);
        }
    }
