...
impl IAMTokenManager {
    ...
    async fn handle_token_refresh(
        ...
        let new_token = Self::generate_token_static(iam_token_state).await?;
        ...
        if let Some(callback) = token_refresh_callback {
            ...
            // Return error for logging purposes in background task, but token refresh succeeded
            Err(GlideIAMError::NoCallbackError)
        }
    }
...
mod tests {
    ...
    async fn test_iam_token_manager_generates_new_token_every_x_seconds() {
        ...
        const REFRESH_TIME_SECONDS: u32 = 3;
